...
const managed = process.argv[1] === ${JSON.stringify(path.join(appRoot, "runtime/python/bin/python3"))};
...
if (args[0] === "--version") { console.log("Python 3.14.4"); }
  ...
  const target = path.join(args[2], "bin", "python3");
...
`);
    try {
      ...
      const commands = (await readFile(log, "utf8")).trim().split("\n").map(line => JSON.parse(line) as { args: string[]; managed: boolean });
      expect(commands.filter(command => command.args[1] === "venv")).toHaveLength(1);
      const pip = commands.filter(command => command.args[1] === "pip");
      expect(pip.every(command => command.managed && !command.args.includes("--user"))).toBe(true);
      if (mode === "success") {
