...
function finalizeRules(logoSet: LogoSetV1): readonly string[] {
  return [
    ...
    `Vectorise the selected candidate into ${LOGO_FILES.logo}: read the PNG, reconstruct its silhouette on a geometric basis (circles and tangents, a modular grid or golden-ratio proportions), apply optical corrections by eye, and keep the result recognisably the same mark. The root <svg> carries viewBox, width and height, and ${LOGO_SOURCE_ATTRIBUTE}="<selected candidate file>". Use <path>, <circle>, <rect>, <ellipse>, <polygon>, <g> and <defs> with local ids only. No <text> (draw letterforms as paths, outlined from a bundled font or the candidate), no <image>, <script>, <foreignObject>, <style> with url(), external href, xlink:href, data: URLs or on* attributes; at most 1 MiB. Solid fills in six-digit hex; no raster effects.`,
