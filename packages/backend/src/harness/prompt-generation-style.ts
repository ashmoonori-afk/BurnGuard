import { COPY_TONE_PRESETS, IMAGE_STYLE_PRESETS, isPhotographicImageStyle, type GenerationStyle } from "@bg/shared";
...
export function appendGenerationStyle(lines: string[], preferences: GenerationStyle | undefined): void {
  ...
  lines.push(isPhotographicImageStyle(preferences.image_style)
    ? "- This is a photographic treatment; keep the realism contract in full: concrete subjects, real settings, camera and light described, no abstract decoration."
    : "- The user explicitly selected a non-photographic treatment; apply it with a concrete subject and composition drawn from the content. Abstract decoration stays prohibited, and the anti-signature and inspection rules of the realism contract still apply.");
