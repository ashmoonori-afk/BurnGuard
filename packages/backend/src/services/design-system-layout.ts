...
type LayoutSystem = { readonly id: string; readonly dir_path: string; readonly tokens_css_path: string | null; readonly readme_md_path: string | null };
...
export async function readDesignSystemLayout(system: LayoutSystem): Promise<DesignSystemLayout> {
  ...
  const [css, readme] = await Promise.all([
    system.tokens_css_path ? readLayoutFile(root, path.relative(root, system.tokens_css_path)) : "",
    system.readme_md_path ? readLayoutFile(root, path.relative(root, system.readme_md_path)) : "",
  ]);
  ...
  const bundled = bundledDesignSystems.find(theme => bundledDesignSystemId(theme.slug) === system.id);
  const original = originalSamples.find(sample => originalSampleSystemId(sample.slug) === system.id);
  const relative = bundled ? ["design system themes", bundled.slug] : original ? ["samples", "original", original.slug, "design-system"] : system.id === "northvale-capital" ? ["design system sample"] : null;
  if (!relative) return local;
  const source = path.join(resolveRepoRoot(), ...relative);
  if (path.resolve(source) === path.resolve(root)) return local;
  const [bundledCss, bundledReadme] = await Promise.all([readLayoutFile(source, "colors_and_type.css"), readLayoutFile(source, "README.md")]);
...
async function readLayoutFile(root: string, relative: string): Promise<string> {
