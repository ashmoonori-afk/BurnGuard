...
| 9. BI | Generated folded-canvas B mark in sidebar, project header, favicon, and both READMEs | `doc/brand-identity.md`, `public/assets/` |
...
Vanilla flags were checked against installed Codex 0.153.4 and Claude Code 2.1.261. Codex ignores user configuration and disables plugin/host-skill discovery and project-document loading. On Windows it explicitly selects the supported unelevated sandbox while keeping `workspace-write`: without that setting the real CLI had returned exit 0 despite rejecting file writes as read-only. The corrected production adapter created and validated a small HTML file in 21.969 seconds using LOW effort and vanilla mode. This timing is a smoke result, not a performance promise. Claude uses safe mode. BurnGuard still supplies its own project brief and generation instructions. Older CLIs may reject these flags and need updating. LOW controls reasoning, not total execution time, network access, or export cost. Claude runs with edit permission and noninteractive permission prompts; this one-shot CLI adapter does not implement an interactive permission round trip.
...
- Full regression: **1,065 passed, 20 skipped, 0 failed**, 139 files and 6,863 assertions. The first pass identified an outdated label expectation and a seed-writer inventory mismatch; both were corrected while retaining the artifact-coordinator guard.
- Browser regression: **32 passed, 0 failed**, including real project creation with retained attachment bytes and roles, unauthenticated graphic rejection, zoom/pan/style scroll/native font changes, comment save/current-model delivery, and actual WebGL scene save/reload/Undo. Comment provider output is explicitly mocked; it is not a provider success claim. The browser pass caught and fixed a live FileList reset bug in the new-project input. A subsequent visual inspection caught the brand asset using an unsupported production static path; it now uses the existing `/assets/` route and portable QA verifies PNG bytes.
- Production frontend and Windows executable build: passed. Three.js loads on demand; the initial JavaScript chunk is approximately 633 kB and the optional Three chunk 552 kB. The 500 kB warning remains visible.
- Windows portable runtime: **6 checks passed** in a relocated Korean/space-containing path with Node/Bun removed from PATH. This includes actual scene GET/PUT/round-trip using the packaged Three bundle and MIT license, inline HTML output, fresh database, native Chromium probe, and served BI PNG signature/content type.
