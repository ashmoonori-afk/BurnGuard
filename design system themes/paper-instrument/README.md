...
- Ground: bright paper with generous emptiness; ink blue marks the action.
- Cover: a framed cover with the instrument beside a stacked title.
- Structure: scale, position and full-width hairline rules; no filled panels, no elevation. One takeaway per slide, titled at `--slide-type-heading` (56px) with support at `--slide-type-body` (34px).
- Imagery: one manufactured object on a plain seamless surface that merges into the paper. At most one image per slide unless the request asks for a grid.
- Never: dark or coloured backdrops, hard directional shadows, cluttered props.

## Content artboards

Each artboard is one fixed frame at the size the request declares. The requested kind's own rules come first - frame sizes, platform exclusion zones, print trim and bleed, and a product detail page's full-height section sequence with its closing call to action - and the rules below govern how each frame or section looks. Keep required content inside the safe inset (7% of the shorter side), place the primary figure at `--content-figure` (0.46) of the shorter side anchored right, and nothing crosses the safe area because `--content-bleed` is `0`.

- Frame: bright paper with generous emptiness; ink blue marks the action.
- Composition: scale, position and full-width hairline rules; no filled panels, no elevation. In a multi-frame set the first frame follows the cover rule above and the last carries the call to action; on a product detail page these rules apply per section.
