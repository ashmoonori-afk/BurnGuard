...
- Ground: cool limestone with ground visible around every plate; slate marks the action.
- Cover: one whole architectural plate with a didone statement beside it.
- Structure: mono captions identifying place, year and material; nothing bleeds off the frame. One takeaway per slide, titled at `--slide-type-heading` (48px) with support at `--slide-type-body` (32px).
- Imagery: an architectural exterior or masonry detail: structure and material, not interiors. At most one image per slide unless the request asks for a grid.
