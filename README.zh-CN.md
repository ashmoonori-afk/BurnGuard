...
本地打包了 72 个字体家族：71 个 Google Fonts 家族加上 Pretendard，各自附带许可证声明。样式面板也可以按需加载系统已安装的字体；本地字体文件不会自动嵌入导出结果。[字体目录](assets/fonts/README.md)。
...
- 即使无法运行渲染设计检查（未安装 Chromium 或大型站点超时），生成回合也会提交，聊天中显示"审查未完成"徽章而不是拒绝该回合。必须修复的问题只在本回合修改过的页面上阻止提交；修改共享 CSS、脚本或图片的回合会检查所有页面。
- 质量面板将不适用于幻灯片和固定尺寸图形的检查标记为"不适用"，测量纯渐变的对比度，将等宽字体视为独立角色，扫描外链样式表中的硬编码颜色，并标记导出时无法获取的远程字体、图片和框架。
- 设计简报跟随应用语言；UX 审查结果、质量/平台修复请求、方向预览以及应用生成的所有 AI 请求均以韩文、英文或中文显示。服务器的每个错误代码都有对应提示。
- 提示词框架理顺了相互矛盾的规则（图表、调色板、断点、社会证明、地图嵌入、仅 Windows 的 PowerShell），新增韩文排版规则，在每个项目内准备 Lucide 图标参考，按请求加入 3D 和图表约定，提供完整的设计系统令牌块，并在精简模式下缩减固定规则。
- 画布：编辑面板可修改图片来源和链接目标，已解决的评论可重新打开，新评论直接打开编辑器，Ctrl/⌘+Z 可撤销样式调整，色板来自页面自身的令牌，列出全部内置字体，并移除了重复的选择模式。
- 主页与设置：深色模式在所有页面生效，语言点击即保存，后端检测失败时可重试，创建成功后清除草稿，搜索覆盖所有项目。
- 网站导入改为移除脚本和表单而非拒绝页面，为提取的令牌记录媒体查询上下文，并生成标准令牌契约；起始模板声明 `--page-background`。

### 0.5.24

- 新项目和内置示例重新可以在画布、实时预览和演示中打开；此前内置字体数量超过了画布字体上限，导致加载失败。
- PDF 和 PNG 压缩包导出保留作者编写的网格/弹性布局和页面背景，并等待延迟加载的图片；内置示例图形也可导出为画板 PDF。超出 PDF 上限的幻灯片会在渲染前说明原因并拒绝。
- Cafe24 和 Imweb 套件接受普通链接，包含所有页面及仅子页面使用的资源，页脚脚本可正常运行，并附带字体许可证；生成套件不再需要 Chromium。
- 导出只包含文档实际使用的内置字体，ZIP 条目使用本地时间，删除项目后不再残留导出缓存。
- 对话会准确显示本轮结果是否真正写入项目（包括幻灯片审校或修复失败）；按需将附件原始页面与幻灯片一一对应。
- macOS 应用：可找到通过 Homebrew 或 `~/.local/bin` 安装的 CLI 和 Python，在浏览器中打开外部链接，支持 ⌘C/⌘V/⌘Q 等标准快捷键，可替换同名下载文件，并在本地服务停止后再退出。
- Windows 应用在可恢复的 WebView2 进程故障时不再退出，并能加载已安装字体。Linux 源码运行也能列出已安装字体；macOS 和 Linux 上每个配置目录只由一个后端使用。
- 设置拆分为共享配置和设备本地的凭据与策略。

### 0.5.23

- 徽标项目实时显示 Codex 图像结果，原样保留候选文件，按画板检查文字，并给出更明确的失败阶段。
- Windows 使用作业对象管理后端进程树，安全传递批处理方式的服务商参数，并按文件名处理原生下载。
- 发布打包前在 Windows 和 macOS 上验证原生画布、持久化和导出。

### 0.5.22

- 新增徽标设计项目：探索轮次、选定的 SVG 母版和品牌规范交付物。
- macOS：可用的沙盒画布、原生导出下载、已安装字体列表，以及用于 PDF 依赖的应用专属 Python 环境。
- 安全修复：解压前检查 PNG 解码尺寸、设计系统发布前重新验证 SVG、收紧 POSIX 包权限。
- 0.5.20 和 0.5.21 未公开发布。

### 0.5.19

- 修复旧开发版本的附件记录阻止服务器启动的问题，仅恢复经验证的记录，保留项目文件和上传原件。
- 读取设计系统规则时统一 Windows 换行符。
- 修复 Windows 浏览器测试启动和测试间干扰，并在 Windows 发布验证中加入旧记录恢复测试。
