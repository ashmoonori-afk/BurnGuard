...
export async function seedPromptSampleDesignSystem(sample: PromptSample, html: string): Promise<void> {
  ...
  const layout = sampleLayoutFiles(sample.layout);
...
export async function seedSplashDesignSystemFiles(dirPath: string, html: string): Promise<void> {
  ...
  const layout = sampleLayoutFiles("splash");
  files["colors_and_type.css"] += "\n" + layout.css;
  files["README.md"] += layout.readme;
  files["SKILL.md"] += " Read and enforce Layout, Composition and Responsive in README.md and all --layout-* tokens.";
