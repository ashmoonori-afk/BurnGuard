...
- Ground: the warm paper ground with white fields; teal is for focus or action.
- Cover: a label-and-value block beside one square evidence frame.
- Structure: label/value rows with explicit required and validation text, keeping the stationery character. One takeaway per slide, titled at `--slide-type-heading` (48px) with support at `--slide-type-body` (32px).
- Imagery: documentary evidence inside a containing hairline frame. At most one image per slide unless the request asks for a grid.
