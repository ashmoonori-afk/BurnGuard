  ...
  return (
    <form className="p-6" onSubmit={(event) => {
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2"><label htmlFor="project-materials" className={PROJECT_LABEL_CLASS}>참고 자료 첨부</label><input id="project-materials" type="file" multiple accept={COMPOSER_SUPPORTED_EXTENSIONS.join(",")} disabled={disabled} onChange={(event) => { const picked = Array.from(event.target.files ?? []); setItems((current) => planAttachmentIntake(current, picked)); event.target.value = ""; }} className="block w-full text-sm" /><p className="text-xs text-muted-foreground">PNG·JPG·WebP·DOCX·PDF·PPTX, 최대 8개 · 파일당 10 MB · 합계 25 MB. 프로젝트 생성 시 docs/attachments에 원본을 저장해요. 직접 전송할 때 AI에 전달하며, 첨부를 빼도 저장된 원본은 남아요.</p><ComposerAttachments items={items} sending={disabled} onRemove={(id) => setItems((current) => current.filter((item) => item.id !== id))} onRoleChange={(id, role) => setItems((current) => setAttachmentRole(current, id, role))} /></div>
        <ProjectBriefFields
          form={form}
          disabled={disabled}
          onChange={update}
