...
export default function FilePreview({ projectId, file }: { projectId: string; file: FileInfo | null }) {
    ...
    return (
      <div className="flex min-h-0 min-w-0 flex-1 items-center justify-center overflow-auto p-6">
        <div className="max-w-xs text-center">
          ...
          <h3 className="text-sm font-medium">{file ? t("files.selectInFolder") : t("files.selectPreview")}</h3>
          <p className="mt-2 text-xs leading-5 text-muted-foreground">{t("files.previewHelp")}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <section aria-label={t("files.preview")} className="flex min-h-0 min-w-0 flex-1 flex-col bg-background">
      <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3 sm:px-6">
        <div className="min-w-0 flex-1 basis-36">
          <p className="mb-1 text-[11px] font-medium text-muted-foreground">{t("files.readOnlyPreview")}</p>
        ...
        <Button asChild variant="outline" className="min-h-11 shrink-0 text-xs">
          <a href={`/api/projects/${encodeURIComponent(projectId)}/fs/${file.rel_path.split("/").map(encodeURIComponent).join("/")}`} download={file.rel_path.split("/").at(-1)}><Download aria-hidden="true" />{t("files.downloadOriginal")}</a>
        </Button>
      </header>
      <div className="min-h-0 flex-1 overflow-auto p-4 sm:p-6" aria-busy={preview.kind === "loading"}>
        {preview.kind === "loading" && <p role="status" className="flex items-center gap-2 text-sm text-muted-foreground"><Loader2 aria-hidden="true" className="h-4 w-4 animate-spin" />{t("files.loading")}</p>}
        {preview.kind === "error" && <div role="alert" className="rounded-xl border border-destructive/25 bg-destructive/5 p-4 text-sm"><div className="flex items-start gap-2"><AlertCircle aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-destructive" /><p className="leading-6">{t(preview.message)}</p></div><Button type="button" variant="outline" onClick={() => setRetry((value) => value + 1)} className="mt-3 min-h-11"><RotateCcw aria-hidden="true" />{t("files.retry")}</Button></div>}
        {preview.kind === "image" && <div className="grid min-h-40 place-items-center rounded-xl border border-border bg-muted/30 p-3"><img src={preview.url} alt={file.rel_path} className="h-auto max-w-full rounded-md" onError={() => setPreview({ kind: "error", message: "files.imageFailed" })} /></div>}
        {preview.kind === "text" && <>
          {preview.truncated && <p role="status" className="mb-3 rounded-lg border border-border bg-muted/40 px-3 py-2 text-xs leading-5 text-muted-foreground">{t("files.truncated")}</p>}
          <pre className="whitespace-pre-wrap break-all rounded-xl border border-border bg-muted/20 p-4 font-mono text-xs leading-6">{preview.text || t("files.emptyFile")}</pre>
        </>}
        {preview.kind === "unsupported" && <div className="rounded-xl border border-dashed border-border p-6 text-center"><File aria-hidden="true" className="mx-auto mb-3 h-6 w-6 text-muted-foreground" /><h4 className="text-sm font-medium">{t("files.useOriginal")}</h4><p className="mx-auto mt-2 max-w-sm text-xs leading-6 text-muted-foreground">{t("files.unsupportedHelp")}</p></div>}
      </div>
    </section>
  );
}
