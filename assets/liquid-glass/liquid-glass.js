(function (root) {
   ...
   *
   * Loaded as a CLASSIC script on purpose, not an ES module: a browser blocks module imports over
   * file://, so an exported artifact opened straight from disk by double-clicking would render
   * nothing. A classic script works from both file:// and http://.
   *
   * Usage in a page:
   *   <script src="liquid-glass/liquid-glass.js"><\/script>
   *   const ctx = canvas.getContext("2d");
   *   const bg = ctx.getImageData(0, 0, canvas.width, canvas.height);
   *   const out = LiquidGlass.renderLiquidGlassRing(bg, { cx: 260, cy: 260, rp: 120 });
   *   ctx.putImageData(new ImageData(out.data, out.width, out.height), 0, 0);
   *
   * Node and bundlers import the ESM wrapper beside this file, liquid-glass.mjs.
   */
  ...
  function refractedRadius(r, options) {
    const rp = options.rp;
   ...
   */
  function renderLiquidGlassRing(background, options) {
  ...
  root.LiquidGlass = { RING_OUTER, SHADOW_OUTER, GLASS_COLOR, lensProfile, refractedRadius, renderLiquidGlassRing };
