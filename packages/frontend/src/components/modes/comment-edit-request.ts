...
const COMMENT_EDIT_INSTRUCTIONS = `${LEGACY_INSTRUCTIONS} 이미지 재생성 요청은 실행 전에 대화와 프로젝트에서 기존 이미지 생성 프롬프트를 확인하고, '기존 프롬프트'와 요청을 반영한 '수정 프롬프트'를 함께 보여 준 뒤 '이 프롬프트로 재생성할까요?'라고 물어보세요. 기존 프롬프트를 찾지 못하면 확인할 수 없다고 명시하고 원문을 지어내지 마세요. 사용자의 명시적인 확인을 받을 때까지 이미지 생성 도구를 호출하거나 파일을 변경하지 마세요. 이미 승인된 수정 프롬프트는 다시 묻지 말고 실행하세요. 승인 후에는 해당 프롬프트로 재생성하고 대상 요소에 적용한 뒤, 수정한 현재 파일을 직접 브라우저로 렌더링하고 스크린샷을 확인하세요. 이미지 로딩, 배치, 비율, 잘림을 확인하고 발견한 문제를 고친 뒤 다시 검증하세요. 이미지 재생성 이외의 수정도 적용 후 같은 렌더링 검증을 직접 수행하세요. 브라우저 실행이 실패하면 허용된 다른 로컬 브라우저나 렌더링 도구를 확인하고 가능한 경로로 재시도하세요. 검증을 사용자에게 떠넘기거나 확인하지 않은 결과를 완료로 보고하지 마세요. 실제로 검증이 막혔다면 시도한 방법과 확인된 원인을 간단히 설명하세요. 사용자에게는 프롬프트 비교와 확인 질문, 또는 변경 파일과 검증 결과만 간결하게 보여 주고 내부 코멘트 ID, 선택자, 리비전, 다이제스트나 아래 JSON을 그대로 노출하지 마세요.`;
...
export function commentEditDisplayText(text: string): string {
  ...
  try {
    ...
    return `코멘트 수정 · ${data.file}${data.slide_index === null ? "" : ` · ${Number(data.slide_index) + 1}번 슬라이드`}\n${data.request}`;
  } catch {
    return text;
  }
}
