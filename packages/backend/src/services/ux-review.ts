...
import { UX_LIMITATION_CODES, type ProjectType, type UxFindingCode, type UxReviewFinding, type UxReviewFindingDetail, type UxReviewReport } from "@bg/shared";
...
export function reviewHtml(html: string, kind: ProjectType): readonly UxReviewFinding[] {
  ...
  // The authored strings are the fallback; the UI localizes by code and interpolates the detail.
  const add = (code: UxFindingCode, node: HTMLElement | null, title: string, evidence: string, proposal: string, pattern_id: string, priority: "high" | "medium" = "medium", detail?: UxReviewFindingDetail) => {
    ...
    findings.push({ id: `${code}:${findings.length}`, code, priority, title, evidence: evidence.slice(0, 240), proposal, node_bg_id, pattern_id, ...(detail === undefined ? {} : { detail }) });
  };
  ...
  let previous = 0;
  for (const node of headings) { const level = Number(node.tagName.slice(1)); if (previous && level > previous + 1) add("heading_jump", node, "제목 단계 검토", `${previous}단계 제목 다음에 ${level}단계 제목이 있습니다.`, "내용의 상하 관계에 맞도록 제목 단계를 조정하세요.", "heading", "medium", { previous, level }); previous = level; }
  for (const node of nodes.filter(visible)) {
    if (node.tagName === "BUTTON" || node.getAttribute("role") === "button") {
      const label = name(node);
      if (!label || GENERIC_ACTION_NAMES.test(label)) add("action_name", node, "버튼의 행동을 구체화", label ? `일반적인 버튼 이름: ${label}` : "버튼의 접근 가능한 이름을 찾지 못했습니다.", "문맥에서 행동과 결과를 알 수 있는 버튼 이름을 작성하세요.", "action", label ? "medium" : "high", label ? { label } : undefined);
    }
    ...
    if (node.tagName === "A" && node.hasAttribute("href") && (!name(node) || GENERIC_LINK_NAMES.test(name(node)))) add("link_name", node, "링크의 목적 명시", name(node) ? `일반적인 링크 이름: ${name(node)}` : "링크의 접근 가능한 이름을 찾지 못했습니다.", "이동할 정보나 목적지를 설명하는 이름으로 바꾸세요.", "link", "medium", name(node) ? { label: name(node) } : undefined);
    ...
    if (node.tagName === "P" && text(node).length > (kind === "slide_deck" || kind === "graphic" ? 180 : 500)) add("long_paragraph", node, "문단의 읽기 부담 검토", `문단 길이가 ${text(node).length}자입니다.`, "필요한 내용을 보존하면서 핵심 문장과 세부 설명을 나눌지 검토하세요.", "reading", "medium", { length: text(node).length });
  }
  return findings;
}

/** Generic names in the three UI languages that say nothing about the action or destination. */
const GENERIC_ACTION_NAMES = /^(ok|submit|click here|확인|클릭|보내기|确定|确认|提交|点击|点击这里|发送)$/i;
const GENERIC_LINK_NAMES = /^(here|more|read more|click here|여기|더 보기|자세히|这里|更多|查看更多|点击这里|详情)$/i;
...
export async function getProjectUxReview(projectId: string, requestedPath?: string): Promise<UxReviewReport> {
  ...
  try {
    ...
    return { schema_version: 1, project_id: projectId, artifact_revision: project.current_revision, artifact_digest: project.current_digest, source_path, basis: "local_html_heuristics", findings, limitations: [...UX_LIMITATION_CODES] };
  } catch (error) {
