...
- Ground: warm off-white paper; buttercup marks the action or the active state.
- Cover: a framed cover panel under a soft lowercase display title.
- Structure: warm hairline dividers and printed, approachable body copy. One takeaway per slide, titled at `--slide-type-heading` (52px) with support at `--slide-type-body` (32px).
- Imagery: everyday life at close range, warm and deliberately unremarkable. At most one image per slide unless the request asks for a grid.
