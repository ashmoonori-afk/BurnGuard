...
/** A fixed size that a document may raise through --bg-chart-min-text; without it the size is unchanged. */
const floor = (px: number) => `max(${px}px,var(--bg-chart-min-text,0px))`;
const palettes = {
...
export function renderChart(input: ChartV1): string {
  ...
  if (chart.type === "sankey") {
    ...
    legend = `<div style="display:flex;flex-wrap:wrap;gap:8px 20px;font-size:${floor(14)};margin:8px 0">${items.map((item, i) => `<span><span aria-hidden="true" style="display:inline-block;width:10px;height:10px;margin-right:7px;background:${color(i)};border-radius:2px"></span>${escape(item)}</span>`).join("")}</div>`;
  }
