...
- Ground: the warm grey ground with soft edged surfaces; one violet marks the action; semantic colour stays inside its chips.
- Cover: a centred statement over a soft square field.
- Structure: a humanist voice at low arousal, brief motion, nothing shouting for attention. One takeaway per slide, titled at `--slide-type-heading` (48px) with support at `--slide-type-body` (32px).
- Imagery: interface fragments and soft abstract forms, implied rather than photographed. At most one image per slide unless the request asks for a grid.
